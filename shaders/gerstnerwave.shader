...
PS
{
	...
	float4 MainPs( PixelInput i ) : SV_Target0
	{
		...
		float l_191 = l_189.y;
		float l_192 = l_189.z;
		float l_193 = 1.3 - l_192;
		float4 l_194 = float4( l_190, l_191, l_193, 0 );
		float4 l_195 = normalize( l_194 );
		float4 l_196 = float4( l_25, 0 ) * l_195;
		float3 l_197 = i.vPositionWithOffsetWs.xyz + g_vHighPrecisionLightingOffsetWs.xyz;
		float3 l_198 = pow( 1.0 - dot( normalize( l_196.xyz ), normalize( l_197 ) ), 1 );
		float4 l_199 = lerp( l_0, l_1, float4( l_198, 0 ) );
